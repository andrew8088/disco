...
`,
  ...
  const rows = z.array(reviewEntryParser).parse(stmt.all());
  ...
  return Object.values(entries).map((entries) => {
    ...
    return { journal_entry_id, date, description, fromAccount, toAccount };
  });
}

export function update(id: Id, changes: Partial<Entry>) {
